...
distribute_tickets(void)
{
	...
	tickets_p = STRIDE_TOTAL_TICKETS / numActive;
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
		if(p->state == RUNNABLE || p->state == RUNNING) {
			p->tickets = tickets_p;
			p->stride = STRIDE_TOTAL_TICKETS * 10 / tickets_p;
			p->pass = 0;
		}
	}
}
...
scheduler(void)
{
  ...
  for(;;){
    ...
    sti();
		...
		if(use_stride) {
			ran = 0;
			struct proc *lowest_pass = ptable.proc;
			int lowest_pass_value = INT_MAX;
			for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
				if(p->state == RUNNABLE && p->pass < lowest_pass_value) {
					lowest_pass = p;
					lowest_pass_value = p->pass;
					ran = 1;
				}
			}
			if(ran != 0) {
				c->proc = lowest_pass;
				lowest_pass->pass += lowest_pass->stride;
				switchuvm(lowest_pass);
				lowest_pass->state = RUNNING;
				
				swtch(&(c->scheduler), lowest_pass->context);
				switchkvm();
...
int transfer_tickets(int pid, int tickets)
{
	...
	struct proc *p = myproc();
	if(tickets > p->tickets - 1) return -2;
	struct proc *r;
	int found = 0;
	...
	r->tickets += tickets;
	r->stride = STRIDE_TOTAL_TICKETS * 10 / r->tickets;
	return p->tickets;
}